classdef read_current < realtime.internal.SourceSampleTime ...
        & coder.ExternalDependency ...
        & matlab.system.mixin.Propagates ...
        & matlab.system.mixin.CustomIcon
    %
    % Current measurements from the Momentum Wheels provided by the Arduino's ADC. current(1): Pitch MW; current(2): Roll MW; current(3): Yaw MW
    % 
    ...
    methods (Access=protected)
        ...
        function current = stepImpl(obj)   %#ok<MANU>
            current = double(zeros(3,1));
            if isempty(coder.target)
                % Place simulation output code here
            else
                % Call C-function implementing device output
                current(1) = coder.ceval('get_current',0);
                current(2) = coder.ceval('get_current',1);
                current(3) = coder.ceval('get_current',2);
            end
        end
